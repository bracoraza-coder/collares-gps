...
window.openAmazonUrl = function(url) {
  if (!url) return;
  try {
    ...
    console.error('Error bypass popup blocker:', err);
    window.open(url, '_blank');
  }
};

window.getProductLink = function(p) {
  if (!p) return `https://www.amazon.es?tag=${state.affiliateTag}`;
  try {
    let link = p.affiliate_link || '';
    
    // If we have a curated affiliate_link, prioritize using it
    if (link) {
      // If it's a full Amazon.es URL, ensure the affiliate tag is dynamically updated
    ...
    // Fallback: If affiliate_link is missing, try constructing with ASIN mapping
    const asin = productAsins[p.id];
    if (asin) {
      return `https://www.amazon.es/dp/${asin}?tag=${state.affiliateTag}`;
    }
    
    return `https://www.amazon.es?tag=${state.affiliateTag}`;
  } catch (err) {
    console.error('Error generating product link:', err);
    return `https://www.amazon.es?tag=${state.affiliateTag}`;
  }
};
...
function parseHash() {
  ...
  if (parts[1]) {
    ...
    if (queryParams.has('id')) {
      params.id = parseInt(queryParams.get('id'), 10);
    }
  }
  return { view, params };
}
...
function init() {
  ...
  // Parse initial hash routing on page load
  ...
  window.addEventListener('hashchange', () => {
    const p = parseHash();
    if (state.currentView !== p.view || JSON.stringify(state.viewParams) !== JSON.stringify(p.params)) {
      state.currentView = p.view;
      state.viewParams = p.params;
      renderApp();
    }
...
function renderApp() {
  ...
  // Initialize view-specific components after DOM insertion safely
  try {
    ...
    console.error('Safe-catch: Error initializing charts on view change:', chartErr);
  }
}
